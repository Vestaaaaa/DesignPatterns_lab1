    ...
    lines.forEach((line) => {
        ...
        try {
            if (parts.length === 6) {
                ...
                logger.info(`Triangle created with area:${triangle.getArea()} and perimeter:${triangle.getPerimeter()}`);
            }
            else if (parts.length >= 8) {
                // Конус
                // apexX apexY apexZ baseX baseY baseZ height radius [id]
                const [apexX, apexY, apexZ, baseX, baseY, baseZ, height, radius, id] = parts;
                const nums = [
                    apexX,
                    apexY,
                    apexZ,
                ...
                logger.info(`Cone created with id:${cone.id}, height:${cone.height}, radius:${cone.radius}`);
            }
            else {
