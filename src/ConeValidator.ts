...
export default class ConeValidator {
  static isValid(cone: Cone): boolean {
    if (!cone) return false;

    if (!(cone.apex && cone.baseCenter)) return false;

    if (typeof cone.height !== "number" || cone.height <= 0) return false;

    if (typeof cone.radius !== "number" || cone.radius <= 0) return false;
    ...
    if (!(baseZIsZero || baseYIsZero || baseXIsZero)) return false;
